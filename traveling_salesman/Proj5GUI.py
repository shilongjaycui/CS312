...
#		self.repaint()
	...
	def addEdge(self, startPt, endPt, label, edgeColor, labelColor=None, xoffset=0.0):
		...
		midp = QPointF((edge.x1()*0.2 + edge.x2()*0.8), (edge.y1()*0.2 + edge.y2()*0.8))
	...
	def paintEvent(self, event):
		...
		for color in self.edgeList:
			...
			for edge in self.edgeList[color]:
				...
				tri_pts.append(QPointF(-arrow_scale*(2*unit_edge[0] + unit_edge_perp[0]), arrow_scale*(2*unit_edge[1] + unit_edge_perp[1])))
				tri_pts.append(QPointF(-arrow_scale*(2*unit_edge[0] - unit_edge_perp[0]), arrow_scale*(2*unit_edge[1] - unit_edge_perp[1])))
...
class Proj5GUI(QMainWindow):
	...
	def generateNetwork(self):
		...
		self._scenario = Scenario(city_coordinates=points, difficulty=diff, random_seed=rand_seed)
	...
	def addCities(self):
		...
		self.view.clearEdges()
		for city in cities:
			self.view.addLabel(QPointF(city.x, city.y), city.name, labelColor=(128, 128, 128), xoffset=10.0)

	def generateClicked(self):
		self.generateNetwork()
		self.view.addPoints([QPointF(c.x, c.y) for c in self._scenario.get_cities()], (0, 0, 0))
	...
	def displaySolution(self):						# what about calling this somehow every time a new bssf is found?
		...
		if self._solution:
			self.addCities()
			edges = self._solution.get_edges()
			if edges:
				edgeColor = (128, 128, 255)
				labelColor = (64, 64, 255)
				for edge in edges:
					pt1, pt2, label = edge
					self.view.addEdge(QPointF(pt1.x, pt1.y), QPointF(pt2.x, pt2.y), '{}'.format(label), edgeColor, labelColor)
		else:
	...
	def solveClicked(self):								# need to reset display??? and say "processing..." at bottom???
		self.solver.set_scenario(self._scenario)
	...
	def checkGenInputs(self):
		...
		if self._scenario:
			if self.genParams['seed'] == seed and self.genParams['size'] == size and self.genParams['diff'] == diff:
	...
	ALGORITHMS = [('Default                            ', 'default'), ('Greedy', 'greedy'), ('Branch and Bound', 'branch_and_bound'), ('Fancy', 'fancy')]  # whitespace hack to get longest to display correctly
