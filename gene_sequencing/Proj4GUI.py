...
#		self.processed_results = self.solver.
		#		sequences,
#													self.table,
...
#													align_length=int(self.alignLength.text()))
		for i in range(len(sequences)):
			...
			for j in range(len(sequences)):
				if(j < i):
					...
					# print('sequence 1 ({}):'.format(type(sequences[i])), sequences[i])
					# print('sequence 2 ({}):'.format(type(sequences[j])), sequences[j])
					s = self.solver.align(sequences[i], sequences[j], banded=self.banded.isChecked(), align_length=int(self.alignLength.text()))
	...
	def cellClicked(self, i, j):
		...
		if self.processed_results and j >= i:
			...
			print('Alignment 1:', results['seqi_first100'])
			print('Alignment 2:', results['seqj_first100'])
