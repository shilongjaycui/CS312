...
class Proj3GUI( QMainWindow ):
	...
	def generateNetwork(self):
		nodes = self.newPoints()	# initialize the list of points (QPointF objects)
		OUT_DEGREE = 3				# ?
		size = len(nodes)			# size == number of nodes in the graph
		edgeList = {}				# initialize the edge dictionaryCS312Graph
		for u in range(size):
			edgeList[u] = []
			pt_u = nodes[u]
			chosen = []
